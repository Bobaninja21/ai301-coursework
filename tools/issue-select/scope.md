...
- Repo: `codepath/pathreview-ai301-fa26-s3` <!-- section repo, from the Unit 1 Check-In page -->
...
I am most comfortable in Python — FastAPI services, SQLAlchemy/Alembic, and
pytest — and I have worked in a full-stack repo with that shape (a Python
API plus a JavaScript/TypeScript frontend), so I can find my way around
both halves. I have also done a fair amount of agent and LLM plumbing:
prompt and skill files, retrieval pipelines, and eval harnesses.

What I want to get better at is reading an unfamiliar codebase quickly and
landing a small, well-tested change in it — so I prefer backend Python
bugs and documentation fixes with a clear reproduction over open-ended
feature design. I am happy to touch the frontend when the change is
scoped to specific components.

What I want to avoid this unit: infrastructure and deployment work
(Docker, CI, Kubernetes) where I cannot verify the fix locally, and
anything that needs a design decision made before the code can be
written. I have a few evenings, not a few weeks.
